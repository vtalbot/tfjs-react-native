...
describeWithFlags("BundleResourceIO", RN_ENVS, () => {
    ...
    it("errors on string modelJSON", async () => {
        const response = new Response(weightData1);
        spyOn(tf.env().platform, "fetch").and.returnValue(Promise.resolve(response));
        const modelJson = `{
      modelTopology: modelTopology1,
      weightsManifest: [{
        paths: [],
        ...
        const resourceId = 1;
        expect(() => bundleResourceIO(modelJson, resourceId)).toThrow(new Error("modelJson must be a JavaScript object (and not a string).\n" +
...
describeWithFlags("BundleResourceIO Sharded", RN_ENVS, () => {
    ...
    it("loads model artifacts", async () => {
        spyOn(tf.env().platform, "fetch").and.returnValues(Promise.resolve(new Response(weightData1)), Promise.resolve(new Response(weightData2)));
        const modelJson = {
            modelTopology,
            weightsManifest: [
                {
